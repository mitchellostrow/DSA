...
class SubspaceDMDc():
    ...
    def all_to_device(self, device):
        ...
        if self.info is not None:
            for key in ['R_hat', 'Q_hat', 'S_hat', 'Gamma_hat', 'singular_values_O', 'noise_covariance']:
    ...
    def subspace_dmdc_multitrial_QR_decomposition(self, y_list, u_list, p, f, n=None, lamb=1e-8, energy=0.999):
        ...
        for trial_idx, (Y_trial, U_trial) in enumerate(zip(y_list, u_list)):
            ...
            U_p_trial = torch.cat([hankel_stack(U_trial, j, p) for j in range(T_trial)], dim=1)
            Y_p_trial = torch.cat([hankel_stack(Y_trial, j, p) for j in range(T_trial)], dim=1)
            U_f_trial = torch.cat([hankel_stack(U_trial, j + p, f) for j in range(T_trial)], dim=1)
            Y_f_trial = torch.cat([hankel_stack(Y_trial, j + p, f) for j in range(T_trial)], dim=1)
        ...
        Q, R_upper = torch.linalg.qr(H.T, mode='reduced')  # H.T = Q R_upper, R_upper upper triangular
        ...
        # 4) Symmetrize
        eps = 1e-12
        R_hat = 0.5 * (R_hat + R_hat.T) + eps * torch.eye(R_hat.shape[0], device=self.device)
        Q_hat = 0.5 * (Q_hat + Q_hat.T) + eps * torch.eye(Q_hat.shape[0], device=self.device)
    ...
    def subspace_dmdc_multitrial_custom(self, y_list, u_list, p, f, n=None, lamb=1e-8, energy=0.999):
        ...
        UfUfT = U_f @ U_f.T
        Xsolve = torch.linalg.solve(UfUfT + lamb*torch.eye(UfUfT.shape[0], device=self.device), U_f)
        Pi_perp = torch.eye(T_total, device=self.device) - U_f.T @ Xsolve
        Yf_perp = Y_f @ Pi_perp
        Zp_perp = Z_p @ Pi_perp
        
        ZZT = Zp_perp @ Zp_perp.T
        Zp_pinv_left = torch.linalg.solve(ZZT + lamb*torch.eye(ZZT.shape[0], device=self.device), Zp_perp)
...
class OnlineKalman:
    ...
    def step(self, y=None, u=None, reg_coef=1e-6):
        ...
        x_filtered : torch.Tensor  
            Filtered state estimate
        """
        x_pred = self.x_predicteds[-1] if self.x_predicteds else torch.zeros((self.x_dim, 1), device=self.device)
        p_pred = self.p_predicteds[-1] if self.p_predicteds else torch.eye(self.x_dim, device=self.device)
        ...
        p_filtered = p_filtered + reg_coef * torch.eye(self.x_dim, device=self.device)  # Add regularization
        ...
        p_predicted = p_predicted + reg_coef * torch.eye(self.x_dim, device=self.device)  # Add regularization
