...
class ConcatObservables(BaseObservables):
    ...
    def transform(self, X):
        ...
        Args:
            X (numpy.ndarray): Measurement data to be fit, with shape (n_samples,
                n_input_features_) or (n_trials, n_samples, n_input_features_).

        Returns:
            y (numpy.ndarray): Evaluation of observables at `X`, with shape (n_samples,
                n_output_features_) or (n_trials, n_samples, n_output_features_).

        Raises:
            NotFittedError: If the model is not fitted yet.
        """

        # Handle 3D data (multiple trials) by processing each trial separately
        if isinstance(X, list):
    ...
    def inverse(self, y):
        ...
        Args:
            y (numpy.ndarray): Data to which to apply the inverse.
                Shape must be (n_samples, n_output_features) or
                (n_trials, n_samples, n_output_features).
                Must have the same number of features as the transformed data.

        Returns:
            numpy.ndarray: Output of inverse map applied to y.
                Shape will be (n_samples, n_input_features) or
                (n_trials, n_samples, n_input_features).
